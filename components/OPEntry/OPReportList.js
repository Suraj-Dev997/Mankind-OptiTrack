...
const OPReportList = () => {
  ...
  const handleInfo = orid => {
    console.log('crid id', orid);
    navigation.navigate('OPCampInfo', {crId: orid, id}); // Pass the doctorId as a parameter
  };

  const handleEdit = orid => {
    console.log('crid id', orid);
    navigation.navigate('OPUpdateCampImages', {crid: orid, id}); // Pass the doctorId as a parameter
  };

  const handleDelete = async orid => {
    console.log('This g', orid);
    try {
      const ApiUrl = `${BASE_URL}${'/operativeReport/deleteReportWithId'}`;
      ...
      if (response.ok) {
        ...
        const updatedUsers = users.filter(user => user.orid !== orid);
      ...
      AsyncStorage.getItem('userdata')
        .then(data => {
          if (data) {
            ...
            const ApiUrl = `${BASE_URL}${'/operativeReport/getAllCampReport'}`;
            return fetch(ApiUrl, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
        ...
        <View style={styles.actionButtons}>
          ...
          <TouchableOpacity style={styles.actionButton}>
            <IconButton
              ...
              iconColor="#dc222d"
              size={20}
              onPress={() => handleDelete(item.orid)}
            />
          </TouchableOpacity>
        </View>
      </View>
        ...
        <View style={styles.headertop}>
          <LinearGradient colors={['#000953', '#092d4f']} style={styles.addbtn}>
            <Button
              ...
              onPress={() => navigation.navigate('OPUploadCampImages', {id})}>
              Add Report
            </Button>
          </LinearGradient>
        </View>
      ...
      <View style={styles.tableCont}>
        ...
        {isLoading ? (
          ...
          <FlatList
            ...
            keyExtractor={item => (item ? item.orid.toString() : '0')} // Updated keyExtractor
          />
        )}
      </View>
      </View>
