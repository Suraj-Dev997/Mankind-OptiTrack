...
const GReportList = () => {
  ...
  const handleInfo = grid => {
    console.log('crid id', grid);
    navigation.navigate('GCampInfo', {crId: grid, id}); // Pass the doctorId as a parameter
  };

  const handleEdit = grid => {
    console.log('crid id', grid);
    navigation.navigate('GUpdateCampReport', {crId: grid, id}); // Pass the doctorId as a parameter
  };

  const handleDelete = async grid => {
    console.log('This g', grid);
    try {
      const ApiUrl = `${BASE_URL}${'/glaucomaReport/deleteReportWithId'}`;
      ...
      if (response.ok) {
        ...
        const updatedUsers = users.filter(user => user.grid !== grid);
      ...
      AsyncStorage.getItem('userdata')
        .then(data => {
          if (data) {
            ...
            const ApiUrl = `${BASE_URL}${'/glaucomaReport/getAllCampReport'}`;
            return fetch(ApiUrl, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
        ...
        <View style={styles.actionButtons}>
          ...
          <TouchableOpacity style={styles.actionButton}>
            <IconButton
              ...
              iconColor="#dc222d"
              size={20}
              onPress={() => handleDelete(item.grid)}
            />
          </TouchableOpacity>
        </View>
      </View>
    ...
    <View style={styles.container}>
        ...
        <View style={styles.headertop}>
          <LinearGradient colors={['#000953', '#092d4f']} style={styles.addbtn}>
            <Button
              ...
              onPress={() => navigation.navigate('GAddCampReport', {id})}>
              Add Report
            </Button>
          </LinearGradient>
        </View>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          keyExtractor={item => (item ? item.grid.toString() : '0')} // Updated keyExtractor
        />
      )}
    </View>
      </View>
